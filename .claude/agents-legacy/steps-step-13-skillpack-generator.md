...
- `sss-planner` (subagent; tool-restricted where appropriate)
...
3. Update `.gitignore` entries for symlinks
...
SKILL_COUNT=$((SKILL_COUNT + $(find .claude/skills -name "SKILL.md" 2>/dev/null | wc -l)))
```

### 7.2 Replace Placeholders in CLAUDE.md

```bash
# Read the generated skill matrix
SKILLS_CONTENT=$(cat /tmp/skill-matrix.md)

# Replace {{SKILLS_BY_CATEGORY}} placeholder in CLAUDE.md
# Replace {{SKILL_COUNT}} with actual count
```
...
If `AGENTS.md` exists, append the skill-to-agent mappings:

```markdown
## Skill-to-Agent Mapping

Based on project skills, here's which agent should use which skills:
...
Confirm placeholders were replaced:

```bash
# Check {{SKILLS_BY_CATEGORY}} was replaced
if grep -q "{{SKILLS_BY_CATEGORY}}" CLAUDE.md; then
  echo "ERROR: Placeholder not replaced"
  exit 1
fi

# Check {{SKILL_COUNT}} was replaced
if grep -q "{{SKILL_COUNT}}" CLAUDE.md; then
  echo "ERROR: Skill count not replaced"
  exit 1
fi

echo "✅ CLAUDE.md skill section populated with $(SKILL_COUNT) skills"
```

**HITL checkpoint →** Confirm skill matrix generation.
**Prompt:** "Generated skill matrix with [N] skills across [M] categories. Reply `confirm` to update CLAUDE.md."
...
- Canonical locations (`docs/.cursor/rules/`, `docs/.claude/skills/`)
...
| Check | Description | Points |
|------|-------------|--------|
| not_contains:CLAUDE.md:{{SKILLS_BY_CATEGORY}} | Skills placeholder was replaced | 5 |
| not_contains:CLAUDE.md:{{SKILL_COUNT}} | Skill count placeholder was replaced | 3 |
| has_pattern:CLAUDE.md:Frontend & UI\|Backend & API | Category tables present | 3 |
| has_pattern:CLAUDE.md:@frontend-design\|@senior-architect | Skill references present | 2 |
| has_pattern:AGENTS.md:Skill-to-Agent Mapping | Agent skill mappings added | 2 |
