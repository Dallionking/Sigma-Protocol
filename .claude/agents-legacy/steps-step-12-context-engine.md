...
- `sigma-planner.md` (mode: `subagent`) — planning (ideally tool-restricted)
...
- Frontmatter keys: `description` (required), optional `agent`, `model`, `subtask`
- Body: the full prompt template
...
**Prompt:** "Will generate `boilerplate-patterns.mdc`. Reply `confirm` to continue."
...
**Prompt:** "Will generate `reasoning.mdc` for epistemic caution. Reply `confirm` to continue."
...
**Prompt:** "Will generate `.sigma/rules/` and `.claude/rules/` with modular rules. Reply `confirm` to continue."
...
<!-- Step 13 auto-generates this section -->
{{SKILLS_BY_CATEGORY}}
...
## Phase 3.5: Generate CLAUDE.md with Swarm-First Section (NEW)
...
[Include full swarm-first section from templates/claude-md/swarm-first-section.md]

## 📚 Agent Registry

[Include agent registry from templates/claude-md/agent-registry.md]

## 🗂️ Skills Registry ({{SKILL_COUNT}} Skills by Category)

{{SKILLS_BY_CATEGORY}}

[Note: Step 13 will replace these placeholders with actual content]
...
5. Leave `{{SKILLS_BY_CATEGORY}}` and `{{SKILL_COUNT}}` as placeholders for Step 13

**HITL checkpoint →** Confirm CLAUDE.md generation.
**Prompt:** "Will generate CLAUDE.md with swarm-first philosophy. Reply `confirm` to continue."
...
4.  **Generate CLAUDE.md**: Write CLAUDE.md with swarm-first section and placeholders.
5.  **Verify**: Check that file paths in `@import` match actual files.
...
|-------|-------------|--------|
| has_pattern:.cursorrules:@import\|import | Context routing configured | 5 |
| file_count:/.cursor/rules/:3 | At least 3 rule files generated | 5 |
| has_pattern:CLAUDE.md:Never work solo | Swarm mandate present | 5 |
| has_pattern:CLAUDE.md:sigma-planner\|sigma-executor | Agent registry present | 5 |
