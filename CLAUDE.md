...
|----------|---------------|--------|--------|
| **Claude Code** | `.claude/` | 151 | Production |
...
| Mode | When | Tool |
|------|------|------|
| **Agent Teams** (native) | Single-session, ≤5 agents, coordinated work | `TeammateTool` + `SendMessage` |
| **Sigma Streams** | Multi-session, complex orchestration, 5-20 agents | `/stream` + `/orchestrate` |
| **Ralph Loop** | Autonomous PRD execution | `sigma-ralph.sh` |
...
### Claude Code v2.1.32 Native Tools

Claude Code v2.1.32 includes native tools that Sigma Protocol leverages:

#### Task Management
| Tool | Description |
|------|-------------|
| `TaskCreate` | Create tasks with dependencies |
| `TaskUpdate` | Update task status/content (supports `status: "deleted"`) |
| `TaskList` | List all tasks with filters |
| `TaskGet` | Get task details by ID |

**Dependency Tracking:** `blockedBy` / `blocks` fields
**Status Workflow:** `pending` → `in_progress` → `completed`
**Multi-session:** Use `CLAUDE_CODE_TASK_LIST_ID` env var to share across sessions

#### Agent Teams (v2.1.32)
Native multi-agent collaboration — replaces manual swarm orchestration for applicable workflows.
- Requires `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1` (set in project `.claude/settings.json`)
- Uses `TeammateTool` + `SendMessage` for coordination
- One team per session, no nested teams
- Complements Sigma's `/stream` and `/orchestrate` commands

**Split-pane setup (recommended):** Each teammate gets its own terminal pane.
Add to your **user** settings (`~/.claude/settings.json`):
```json
{ "teammateMode": "tmux" }
```
Requires tmux (`brew install tmux`) or iTerm2 with tmux integration.
Values: `"tmux"` (force split panes), `"auto"` (detect environment), `"in-process"` (single terminal).

**Auto iTerm2 panes (macOS):** A PostToolUse hook on `Teammate` automatically creates labeled iTerm2 split panes when teams spawn. Panes appear dynamically as each teammate joins. Falls back silently in VS Code, tmux mode, or non-macOS environments. Manual launcher: `scripts/team-iterm-launcher.sh --help`.

#### Memory System (v2.1.32)
Auto-records and recalls context across sessions.
- Stored at `~/.claude/projects/<project-hash>/<session-id>/session-memory/`
- Complements SLAS — Memory handles raw recall, SLAS handles preference distillation

#### Tool Search (v2.1.7+)
Auto-defers MCP tools when descriptions exceed context threshold.
- Set via `ENABLE_TOOL_SEARCH=auto:5` (5% of context)
- Use `ToolSearch` to discover and load deferred tools on-demand
- 85% reduction in token overhead from MCP tool descriptions

#### Additional Capabilities
| Feature | Version | Usage |
|---------|---------|-------|
| **PDF pages** | v2.1.30 | `Read` tool accepts `pages: "1-5"` for large PDFs |
| **Setup Hook** | v2.1.10 | Triggered by `--init` / `--maintenance` flags |
| **PreToolUse additionalContext** | v2.1.9 | Hooks can inject context back to model |
| **PR auto-linking** | v2.1.27 | `--from-pr` flag, PR state in footer |
| **Keybindings** | v2.1.18 | `/keybindings` command, `~/.claude/keybindings.json` |
| **Opus 4.6** | v2.1.32 | Latest model, 1M token context for eligible orgs |

---

## Self-Learning Agent System (SLAS)

SLAS enables AI agents to learn from session history and adapt to developer preferences.

> **Note:** Claude Code v2.1.32 includes a native Memory System that auto-records session context. SLAS complements this — native memory handles raw session recall while SLAS handles preference distillation, pattern recognition, and cross-platform sync.
