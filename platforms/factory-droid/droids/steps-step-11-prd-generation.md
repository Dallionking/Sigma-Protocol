...
**HITL checkpoint →** Confirm boilerplate API approach.
**Prompt:** "PRDs will reference boilerplate stable APIs. Reply `confirm` to continue, or `full spec` to specify everything."
...
```typescript
// actions/[feature]/get-[entity]-by-id.ts
"use server"
...
export async function get[Entity]ById(id: string): Promise<Get[Entity]Result> {
  try {
    // Validate input
    const validated = inputSchema.parse({ id })
...
```typescript
// actions/[feature]/update-[entity].ts
"use server"
...
export type Update[Entity]Input = z.infer<typeof updateSchema>

export async function update[Entity](input: Update[Entity]Input): Promise<{ success: boolean; error?: string }> {
  try {
...
```typescript
// actions/[feature]/delete-[entity].ts
"use server"
...
export async function delete[Entity](id: string): Promise<{ success: boolean; error?: string }> {
  try {
    ...
    // Soft delete or hard delete with ownership check
    const result = await db.delete(tableName)
      .where(and(
        eq(tableName.id, validated),
        eq(tableName.userId, user.id)
      ))
      .returning({ id: tableName.id })
...
> "PRD generated at `docs/prds/F[N]-[FEATURE-NAME].md`.
> 
> **Quality Gates Passed:**
> - [ ] Completeness Gate: All 14 sections complete
