...
### Animation Library Decision Tree (Reference Guide)
...
What do you need?
│
├─► Text Animation (headline, reveal, typing)?
│   ├─► Character-by-character reveal → Motion Primitives `text-effect`
│   ├─► Typewriter effect → KokonutUI `typing-text`
│   ├─► Glitch/Matrix effect → KokonutUI `glitch-text`, `matrix-text`
│   ├─► Shimmer loading → KokonutUI `shimmer-text`
│   └─► Blur fade → Magic UI `blur-fade`
│
├─► Number Animation (stats, counters)?
│   ├─► Smooth flow transitions → `@number-flow/react` ⭐
│   ├─► Counting up → Motion Primitives `animated-number`
│   └─► Sliding digits → Motion Primitives `sliding-number`
│
├─► Button Animation?
│   ├─► Particle explosion → KokonutUI `particle-button`
│   ├─► Magnetic hover → KokonutUI `magnet-button`
│   ├─► Pulsating → Magic UI `pulsating-button`
│   └─► Gradient animated → KokonutUI `gradient-button`
│
├─► Navigation Animation?
│   ├─► macOS-style dock → Motion Primitives `dock` or Cult UI `dock` ⭐
│   ├─► Morphing nav → KokonutUI `morphic-navbar`
│   └─► Smooth tabs → KokonutUI `smooth-tab`
│
├─► Page Transitions? (Web Only)
│   ├─► Native feel → `next-view-transitions` ⭐
│   ├─► Animated layout → `framer-motion` (App Router Template)
│   └─► Progress bar → `react-transition-progress`
│
├─► Input/Form Animation?
│   ├─► Focus border trail → Motion Primitives `border-trail` ⭐
│   ├─► Glow on focus → Motion Primitives `glow-effect` ⭐
│   └─► AI-powered input → KokonutUI `ai-input-*`
│
├─► Card/Container Animation?
│   ├─► Mouse tracking → KokonutUI `mouse-effect-card`
│   ├─► Magic shine → Magic UI `magic-card`
│   ├─► Card flip stack → KokonutUI `card-flip-stack`
│   ├─► Liquid glass → KokonutUI `liquid-glass`
│   └─► 3D tilt → Motion Primitives `tilt`
│
├─► Background Animation?
│   ├─► Animated beams → KokonutUI `beams-background`
│   ├─► Flickering grid → Magic UI `flickering-grid`
│   ├─► Ripple effect → Magic UI `ripple`
│   ├─► Path animations → KokonutUI `background-paths`
│   └─► Gradient mesh → Custom (see MOBILE_APP_DESIGN_LEARNINGS.md)
│
├─► Dialog/Modal Animation?
│   ├─► Shape-morphing → Motion Primitives `morphing-dialog`
│   ├─► Drawer (mobile-style) → `vaul` (shadcn drawer)
│   ├─► Smooth drawer → KokonutUI `smooth-drawer`
│   └─► Standard animated → Motion Primitives `dialog`
│
├─► Loading Animation?
│   ├─► Premium loaders → KokonutUI `loader`
│   ├─► AI processing → KokonutUI `ai-state-loading`
│   └─► Skeleton shimmer → Custom (see patterns)
│
└─► Screen/Navigation Animation? (Mobile Only)
    ├─► Instagram-style photo zoom → Screen Transitions `SharedIGImage`
    ├─► Apple Music album expand → Screen Transitions `SharedAppleMusic`
    ├─► Twitter/X image viewer → Screen Transitions `SharedXImage`
    ├─► Bottom sheet modal → Screen Transitions `SlideFromBottom`
    ├─► Swipe-to-dismiss card → Screen Transitions `DraggableCard`
    └─► Custom gesture-driven → Screen Transitions `screenStyleInterpolator`
```
...
### KokonutUI Component Catalog

**AI Components:**
- `ai-input-selector` - AI input with suggestions
- `ai-input-search` - AI-powered search
- `ai-state-loading` - AI loading states
- `ai-text-loading` - AI text generation loading
- `ai-voice` - Voice input component

**Buttons:**
- `particle-button` - Particle explosion effect
- `v0-button` - V0-style button
- `gradient-button` - Gradient animated button
- `magnet-button` - Magnetic hover effect
- `hold-button` - Hold to confirm
- `social-button` - Social login buttons
- `command-button` - Command palette button
- `switch-button` - Animated switch

**Text Effects:**
- `scroll-text` - Scroll-triggered text
- `typing-text` - Typewriter effect
- `matrix-text` - Matrix rain text
- `dynamic-text` - Dynamic text swap
- `glitch-text` - Glitch effect
- `shimmer-text` - Shimmer loading text
- `sliced-text` - Sliced reveal
- `swoosh-text` - Swoosh animation

**Other Components:**
- `slide-text-button` - Sliding text button
- `loader` - Premium loaders
- `liquid-glass` - Glass morphism
- `action-search-bar` - Action search
- `beams-background` - Animated beams
- `background-paths` - Path animations
- `morphic-navbar` - Morphing navigation
- `mouse-effect-card` - Mouse tracking cards
- `bento-grid` - Bento grid layouts
- `card-flip-stack` - Card stack animations
- `file-upload` - Premium file upload
- `profile-dropdown` - Profile menu
- `smooth-tab` - Smooth tab transitions
- `smooth-drawer` - Smooth drawer

### Motion Primitives Component Catalog

**⭐ Animation Quality Benchmark (Reference Examples)**

These components represent the **level of polish and animation quality** preferred for premium apps. They're NOT mandatory for every project — use them when they fit the app's needs and design direction.

| Component | What It Demonstrates | When to Consider |
|-----------|---------------------|------------------|
| **`border-trail`** | Subtle, alive interactions | Apps with forms, inputs, or cards needing premium focus states |
| **`dock`** | Physics-based navigation | Apps where bottom nav is core to the experience |
| **`animated-number`** | Data that feels dynamic | Dashboards, stats-heavy apps, progress tracking |
| **`text-effect`** | Headlines that captivate | Onboarding flows, landing pages, hero sections |
| **`glow-effect`** | Premium hover polish | Card-based UIs, interactive galleries |

> **Design Philosophy:** These examples show the *direction* — polished, alive, intentional motion. Evaluate each app's needs and apply similar-quality animations where appropriate, not as a blanket checklist.
...
- `text-loop` - Looping text
- `text-morph` - Morphing text
- `text-roll` - Rolling text
- `text-scramble` - Scramble reveal
- `text-shimmer` - Shimmer effect
- `text-shimmer-wave` - Wave shimmer

**Number Effects:**
- `animated-number` - Counting animations
- `sliding-number` - Sliding digits
...
- `animated-group` - Group animations
- `border-trail` - Border animations ⭐
- `carousel` - Smooth carousel
- `cursor` - Custom cursors
- `dialog` - Animated dialogs
- `disclosure` - Expandable sections
- `in-view` - Scroll-triggered animations
- `infinite-slider` - Infinite scroll
- `transition-panel` - Panel transitions

**Advanced:**
- `morphing-dialog` - Shape-morphing dialogs
- `morphing-popover` - Morphing popovers
- `progressive-blur` - Progressive blur effect
- `toolbar-dynamic` - Dynamic toolbars
- `toolbar-expandable` - Expanding toolbars
...
**HITL Checkpoint →** Present template options to user.
...
**HITL Checkpoint →** Present Step-4 import summary.
**Prompt:**
```
...
Reply `confirm import` to proceed with Flow Discovery.
```
...
**HITL Checkpoint →** Present flow inventory.
**Prompt:** 
```
...
Approve flow list? Reply `approve flows` or `revise: [changes]`.
...
**HITL Checkpoint →** Ask platform question.
**Prompt:**
```
Which platform are you building for?
...
Approve this flow list? Reply `approve flows` or `revise: [changes]`.
```
...
**HITL Checkpoint →** Present PRD for approval.
**Prompt:**
```
## Flow PRD: [Flow Name]
...
**HITL Checkpoint → PRD Ready for Your Implementation**
...
**HITL Checkpoint → Flow PRD Complete**
...
**HITL Checkpoint →** Final approval with zero omission proof.
**Prompt:**
```
## Step 5 Complete! 🎉
