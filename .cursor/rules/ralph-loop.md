...
- Generate on-demand reports: `./scripts/ralph/generate-progress-report.sh`
...
```bash
./scripts/ralph/sigma-ralph.sh --workspace=/path --parallel
```
...
```bash
./scripts/ralph/generate-progress-report.sh docs/ralph/prototype/prd.json
```
...
./scripts/ralph/sigma-ralph.sh --workspace=/path --mode=implementation
```
