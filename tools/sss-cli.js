...
async function buildCursor(targetDir, modules, spinner) {
  ...
  for (const module of modules) {
    ...
    if (await fs.pathExists(moduleSource)) {
      const destDir = module === 'steps' ? path.join(outputDir, 'steps') : path.join(outputDir, module);
  ...
  for (const module of modules) {
    ...
    if (await fs.pathExists(moduleSource)) {
      ...
      for (const file of files) {
        ...
        const claudeCodeContent = transformToClaudeCodeAgent(originalContent, file, module);
...
function transformToClaudeCodeAgent(originalContent, filename, module) {
  ...
  if (frontmatterMatch) {
    ...
    existingFrontmatter.version = yamlContent.match(/version:\s*"?([^"\n]+)"?/)?.[1];
    existingFrontmatter.description = yamlContent.match(/description:\s*"?([^"\n]+)"?/)?.[1];
  ...
  for (const module of modules) {
    ...
    if (await fs.pathExists(moduleSource)) {
      ...
      for (const file of files) {
        ...
        const openCodeCommand = transformToOpenCodeCommand(originalContent, file, module);
        await fs.writeFile(
          path.join(targetDir, config.commandsDir, `${file}.md`),
          openCodeCommand
        );
        ...
        const openCodeAgent = transformToOpenCodeAgent(originalContent, file, module);
        await fs.writeFile(
...
async function installCommand(options) {
  ...
  for (const platform of platforms) {
    const spinner = ora(`Installing for ${PLATFORMS[platform].name}...`).start();
    ...
    try {
      ...
      spinner.fail(`${PLATFORMS[platform].name} installation failed: ${error.message}`);
    }
  }
...
async function buildCommand(options) {
  ...
  const modules = ['steps', 'audit', 'dev', 'ops', 'deploy', 'generators', 'marketing'];
...
program
  ...
  .option('-p, --platform <platform>', 'Platform to build (cursor, claude-code, opencode, all)', 'all')
  .option('-t, --target <directory>', 'Target directory', process.cwd())
  .action(buildCommand);
...
program
  ...
  .action(() => {
    console.log(chalk.yellow('Update command coming soon. For now, reinstall with:'));
